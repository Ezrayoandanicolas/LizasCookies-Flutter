...
final dailyPnlProvider =
  ...
  final parts = dateRange.split('|');
  final res = await dio
      .get('/superadmin/dashboard/daily-profit-loss', queryParameters: {
    ...tenantQp,
    'start_date': parts[0],
    'end_date': parts[1],
  });
...
class _AnalyticsPageState extends ConsumerState<AnalyticsPage> {
              ...
              loading: () => SizedBox(
                  ...
                  child: Center(
                      child: CircularProgressIndicator(
                          color: Theme.of(context).colorScheme.primary))),
              error: (e, _) => _errorBox('Gagal memuat statistik',
                  () => ref.invalidate(dashboardStatsProvider(_dateRange))),
            ...
            pnlAsync.when(
              loading: () => SizedBox(
                  ...
                  child: Center(
                      child: CircularProgressIndicator(
                          color: Theme.of(context).colorScheme.primary))),
              error: (e, _) => _errorBox('Gagal memuat data P&L',
                  () => ref.invalidate(dailyPnlProvider)),
          ...
          Row(
            children: List.generate(_ranges.length, (i) {
              ...
              return Expanded(
                child: Padding(
                  padding:
                      EdgeInsets.only(right: i < _ranges.length - 1 ? 8 : 0),
  ...
  Widget _buildStatsSection(Map<String, dynamic> data) {
    ...
    return GridView.count(
      crossAxisCount:
          Responsive.gridColumns(context, mobile: 2, tablet: 2, desktop: 4),
      shrinkWrap: true,
      ...
      children: [
        _statCard('Total Pendapatan', _formatCurrency(revenue), _green,
            Icons.trending_up_rounded),
        _statCard('Total Pengeluaran', _formatCurrency(expenses), _red,
            Icons.money_off_rounded),
        _statCard('Laba Bersih', _formatCurrency(netProfit), _blue,
            Icons.account_balance_wallet_rounded),
        _statCard('Total Pesanan', '$orderCount', _orange,
            ...
            children: [
              const Text('Ringkasan',
                  style: TextStyle(
                      ...
                      color: Colors.black87)),
              const SizedBox(height: 10),
              _summaryRow('Pendapatan Kotor',
                  (summary['gross_revenue'] ?? 0).toDouble(), _green),
              _summaryRow('Total Diskon',
                  -((summary['total_discounts'] ?? 0).toDouble()), _red),
              _summaryRow(
                  'COGS', -((summary['total_cogs'] ?? 0).toDouble()), _red),
              _summaryRow('Total Pengeluaran',
                  -((summary['total_expenses'] ?? 0).toDouble()), _red),
              const Divider(height: 20),
              _summaryRow(
                  'Laba Bersih', (summary['net_profit'] ?? 0).toDouble(), _blue,
                  bold: true),
              ...
              Row(
                children: [
                  ...
                  _badge('${summary['expense_count'] ?? 0} pengeluaran',
                      Colors.grey),
                ],
              ),
            ],
    ...
    return Container(
      ...
      child: Column(
        ...
        children: [
          ...
          _detailRow(
              'Pendapatan', _formatCurrency(revenue), Colors.grey.shade700),
          const SizedBox(height: 4),
          ...
          _detailRow(
              'Profit', _formatCurrency(profit), isPositive ? _green : _red,
              bold: true),
        ],
      ),
    );
